...
export default function ExamCard({ exam, link, key }: ComponentProps) {
  return (
    <li key={key}>
      {/* Exam Questions Link */}
      <Link href={link} className="flex items-center justify-between p-4 bg-blue-50">
        {/* Exam title */}
        <h3>
          {/* Label */}
        ...
        <p className="flex  items-center gap-x-2">
          ...
          {/* Label */}
          <span className="text-sm font-medium m-0 p-0">Duration : {exam?.duration} minutes</span>
        </p>
      </Link>
    </li>
  );
