...
export default function ProfileMenu() {
  ...
  return (
    <div ref={menuRef} className="flex items-center justify-between w-full relative">
      {/* User Profile Section */}
      <div className="flex items-center gap-2.5">
        {/* Profile Image */}
        <Image
          src={profileImage}
          alt={session?.user?.firstName || "Profile picture"}
          width={56}
          height={56}
        ...
        <div className="flex flex-col w-48">
          <p className="text-blue-600 font-medium text-base">{session?.user?.firstName}</p>
          <p className="text-gray-500 text-sm leading-snug">{session?.user?.email}</p>
        </div>
      </div>
