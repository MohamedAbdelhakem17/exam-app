...
export default function CreatePasswordStep() {
    ...
    const form = useForm<CreatePasswordValues>({
        defaultValues: { password: "", rePassword: "", email: sessionStorage.getItem("email") || "" },
    ...
    const onSubmit: SubmitHandler<CreatePasswordValues> = async (data) => {
        await resetPassword(data, {
            onSuccess: () => {
                toast.custom(() => <AppToaster massage="Password successfully updated! You can now sign in." />, { duration: 1000 })
    ...
    return (
        <>
            ...
            <FormLayout label="Create a New Password" resetPassword={true}>
                {/* Description */}
                <p className="pb-6 mb-4 mt-2.5 text-gray-500">Create a new strong password for your account.</p>
